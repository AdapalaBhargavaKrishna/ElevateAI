...
function pickFirstString(obj: GenericRecord, keys: string[]): string {
...
function firstUrl(values: string[]): string {
...
function extractProfileLinks(parsed: GenericRecord): {
    website: string;
    github: string;
    linkedin: string;
    leetcode: string;
    ...
    for (const candidate of fallbackCandidates) {
        ...
        if (!github && candidate.includes("github.com")) github = candidate;
        if (!linkedin && candidate.includes("linkedin.com")) linkedin = candidate;
        if (!leetcode && candidate.includes("leetcode.com")) leetcode = candidate;
    }

    const website = allUrls.find(
        (url) => !url.includes("github.com") && !url.includes("linkedin.com") && !url.includes("leetcode.com")
    ) || "";

    return { website, github, linkedin, leetcode };
}

function mapResumeToProfile(parsedResume: unknown) {
...
export async function importUserInfoFromResume(req: Request, res: Response) {
    try {
