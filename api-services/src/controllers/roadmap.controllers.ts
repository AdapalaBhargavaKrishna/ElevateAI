...
function normalizePhaseProgress(roadmapData: any, rawPhaseProgress: string): PhaseProgressItem[] {
    let parsed: any[] = [];
    try {
    ...
    return (roadmapData?.phases || []).map((phase: any, idx: number) => {
        const existing = parsed.find((p: any) => p?.phaseNumber === phase.phase_number);
        return {
...
export async function generateRoadmap(req: Request, res: Response) {
    try {
        ...
        await prisma.assessment.createMany({
            data: roadmapData.phases.map((phase: any) => {
                const batchItem = assessmentByPhase.get(phase.phase_number);
                return {
            ...
            roadmap: {
                ...
                assessments: roadmap.assessments
                    ...
                    .map((a) => {
                        ...
                        const phaseData = roadmapData.phases.find((p: any) => p.phase_number === a.phaseNumber);
    ...
    try {
        ...
        const assessmentsWithStatus = roadmap.assessments
            ...
            .map((assessment) => {
                ...
                const phaseData = roadmapData.phases.find((p: any) => p.phase_number === assessment.phaseNumber);
                const checklistTotal = Array.isArray(phaseData?.goals) ? phaseData.goals.length : 0;
...
export async function getAssessmentById(req: Request, res: Response) {
    try {
        ...
        const currentPhase = roadmapData.phases.find((p: any) => p.phase_number === assessment.phaseNumber);
...
export async function updateRoadmapProgress(req: Request, res: Response) {
    try {
        ...
        const phase = roadmapData.phases.find((p: any) => p.phase_number === phaseNumber);
        if (!phase) {
