                    ...
                    {badge && (
                        <Badge
                            ...
                            className={`
                                ...
                                ${badge === 'AI' ? 'bg-primary/10 text-primary hover:bg-primary/10' : ''}
        ...
        <TooltipProvider delayDuration={0}>
            <Tooltip>
                <TooltipTrigger asChild>
                    <Link href={to} onClick={handleClick}>{content}</Link>
                </TooltipTrigger>
                <TooltipContent side="right" className="flex items-center gap-2">
                    <span>{label}</span>
                    {badge && (
                        <Badge variant="secondary" className="text-[10px] px-1 py-0 h-4 text-xs">
                            {badge}
                        </Badge>
                    )}
                </TooltipContent>
...
// Reusable nav section for mobile (no collapse logic needed)
const MobileNavSection = ({
    items,
    pathname,
    setMobileOpen,
}: {
    items: typeof toolsNav;
    pathname: string;
        ...
        {items.map((item) => (
            <Link key={item.to} href={item.to} onClick={() => setMobileOpen(false)}>
                <div
                    ...
                    `}
                >
                    <item.icon className={`h-5 w-5 shrink-0 ${pathname === item.to ? 'text-primary' : ''}`} />
                    ...
                    {item.badge && (
                        <Badge
                            ...
                            className={`
                                text-[10px] px-1.5 py-0.5 h-5
                                ${item.badge === 'AI' ? 'bg-primary/10 text-primary hover:bg-primary/10' : ''}
    ...
    const DesktopSidebar = () => (
        ...
        >
            ...
            <div className="flex items-center h-16 border-b border-border px-3 gap-2.5">
                ...
                >
                    {collapsed ? (
                        <Image src="/favicon.png" alt="Elevate AI" width={40} height={40} className="invert dark:invert-0" />
                    ) : (
                        <Image src="/logo.png" alt="Elevate AI" width={100} height={40} className="invert dark:invert-0 object-contain" />
                    )}
                </div>
            ...
            <ScrollArea className="flex-1">
                <nav className="px-2 py-4 space-y-1">
                    ...
                    <div className="space-y-0.5">
                        {mainNav.map((item) => <SidebarNavItem key={item.to} {...item} />)}
                    </div>
                    ...
                    <div className="space-y-0.5">
                        {toolsNav.map((item) => <SidebarNavItem key={item.to} {...item} />)}
                    </div>
                    ...
                    <div className="space-y-0.5">
                        {accountNav.map((item) => <SidebarNavItem key={item.to} {...item} />)}
                    </div>
                </nav>
            </ScrollArea>
            ...
            <div className="p-2 border-t border-border">
                <div className={`flex items-center gap-3 rounded-lg hover:bg-muted/50 transition-colors cursor-pointer ${collapsed ? 'justify-center px-2 py-2.5' : 'px-3 py-2'}`}>
                    <Avatar className="h-8 w-8">
                        <AvatarFallback className="bg-primary text-primary-foreground text-xs">BK</AvatarFallback>
                    </Avatar>
                    <AnimatePresence>
                        {!collapsed && (
                            <motion.div
            ...
            <SheetContent side="left" className="w-[280px] p-0">
                <div className="flex flex-col h-full">
                    ...
                    <ScrollArea className="flex-1">
                        <nav className="px-2 py-4 space-y-1">
                            {/* Main */}
                            <p className="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/60">Main</p>
                            <MobileNavSection items={mainNav} pathname={pathname} setMobileOpen={setMobileOpen} />

                            <Separator className="my-4" />

                            {/* Tools */}
                            <p className="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/60">Tools</p>
                            <MobileNavSection items={toolsNav} pathname={pathname} setMobileOpen={setMobileOpen} />

                            <Separator className="my-4" />

                            {/* Account */}
                            <p className="px-3 mb-2 text-[11px] font-semibold uppercase tracking-wider text-muted-foreground/60">Account</p>
                            <MobileNavSection items={accountNav} pathname={pathname} setMobileOpen={setMobileOpen} />
                        </nav>
                    </ScrollArea>
