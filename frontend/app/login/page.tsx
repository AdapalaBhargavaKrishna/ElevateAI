...
import { Eye, EyeOff, Sun, Moon, Shield, ArrowUpRight } from "lucide-react";
...
function LoginPageContent() {
    ...
    useEffect(() => {
        ...
        if (urlError === "email_exists_local") {
                ...
                <div className="relative z-10 flex flex-col justify-between h-full p-10 xl:p-14">
                    ...
                    <div className="flex-1 flex flex-col justify-center max-w-md">
                        ...
                        <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}
                            ...
                            Prepare smarter.<br /><span className="text-white/40">Interview better.</span>
                        </motion.h1>
                        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }}
                            ...
                            Multi-agent AI coaching that adapts to your career goals, with real-time feedback and personalized growth tracking.
                        </motion.p>
                        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.6 }}
                            className="flex gap-8">
                            {[{ value: "5", label: "AI Agents" }, { value: "24/7", label: "Availability" }, { value: "Live", label: "Feedback" }].map((stat, i) => (
                                <div key={i}>
                                    <p className="text-2xl font-bold text-white tracking-tight">{stat.value}</p>
                                    <p className="text-[11px] text-white/35 font-medium uppercase tracking-wider mt-0.5">{stat.label}</p>
                                </div>
                            ))}
                        </motion.div>
                    </div>
            ...
            <div className="relative flex-1 flex flex-col min-h-screen lg:min-h-0">
                <div className="relative z-20 flex items-center justify-between p-6 lg:px-10">
                    ...
                    <div className="hidden lg:flex items-center gap-1 text-sm text-muted-foreground">
                        ...
                        <Link href="/signup" className="text-foreground font-semibold hover:text-primary transition-colors ml-1">Sign up</Link>
                    </div>
                ...
                <div className="relative z-10 flex-1 flex items-center justify-center px-6 sm:px-10">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
                        ...
                        <div className="mb-8">
                            ...
                            <p className="text-[13px] text-muted-foreground mt-1.5">Welcome back — enter your credentials below</p>
                        </div>
                        ...
                        <p className="text-center text-[13px] text-muted-foreground mt-8 lg:hidden">
                            Don&apos;t have an account?{" "}
                            <Link href="/signup" className="text-foreground font-semibold hover:text-primary transition-colors">Sign up</Link>
                        </p>
                    </motion.div>
                </div>
